...
const CarDetails = () => {
  ...
  useEffect(() => {
    const fetchVehicle = async () => {
      const res = await axios.get(
        `http://localhost:3000/api/user/vehicles/${id}`
      );
  ...
  const handlePrev = () => {
    ...
    setMainImage(
      vehicle.images[
        (currentImageIndex - 1 + vehicle.images.length) % vehicle.images.length
      ]?.url
    );
  };
  ...
  const handleNext = () => {
    ...
    setMainImage(
      vehicle.images[(currentImageIndex + 1) % vehicle.images.length]?.url
    );
  };
    ...
    <>
      <div className="flex flex-col lg:flex-row gap-8 max-w-7xl mx-auto px-4 py-8 bg-[#FFFCF2] min-h-screen">
        ...
        <div className="lg:w-2/3">
          ...
          {/* Description */}
          <div className="mt-8">
            <h2 className="text-2xl font-semibold text-[#403D39] mb-4">
              Description
            </h2>
        ...
        <div className="lg:w-1/3 lg:sticky lg:top-8 self-start">
          ...
          <p className="text-center text-[#403D39] mb-4">
            Offer ID #{vehicle.offerId}
          </p>
      ...
      <section className="max-w-7xl mx-auto px-4 py-8">
        {/* Heading above */}
        <div>
          <h2 className="text-2xl font-semibold text-[#403D39] mb-6 text-center">
            Send Us a Message
          </h2>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className="md:w-1/2">
            <div className="bg-[#EFF3FA] p-8 rounded-2xl shadow-md">
              <AdminCard />
            </div>
          </div>
        </div>
